...
contract LexART721 is ERC721, ERC721Metadata, Ownable {
    ...
    function createArt(
        ...
        // Update data according to token Id
    ...
    function giftLexART(uint256 tokenId, address payable newOwner) public payable {
        ...
        require(newOwner != ownersPerTokenId[tokenId][currentOwnerPerTokenId], "Owner cannot gift to herself!");


        _transferFrom(ownersPerTokenId[tokenId][ownersPerTokenId[tokenId].length - 1], newOwner, tokenId);

        // Add new owner to owners, ownersPerTokenId & ownerIndex, mapping
        ownersPerTokenId[tokenId].push(newOwner); // Increments owner array for selected tokenId
    ...
    ***************/
    ...
    // This transfer does NOT add newOwner to any owner mapping
    function lexDAOtransfer(uint256 tokenId, address currentOwner, address newOwner) public onlyLexDAO {
        _transferFrom(currentOwner, newOwner, tokenId);
    }

    // DAO can vote to burn this token
    function lexDAOburn(uint256 tokenId) public onlyLexDAO {
        _burn(tokenId);
    }

    // DAO can vote to update starting royalties to decay from
    function updateStartingRoyalties(uint8 _startingRoyalties) public onlyLexDAO {
        startingRoyalties = _startingRoyalties;
    }

    // DAO can vote to update artwork hash
    function updateArtworkMetadata(uint256 tokenId, string memory _artworkMetadata) public onlyLexDAO {
        artworkMetadataMapping[tokenId] = _artworkMetadata;
    }

    // DAO can vote to update certificate hash
    function updateCertificateHash(uint256 tokenId, string memory _certificateHash) public onlyLexDAO {
        certificateHashMapping[tokenId] = _certificateHash;
    }
}
