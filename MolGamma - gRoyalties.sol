...
contract MolGamma {
    ...
    function mintDashboard(
            ...
            gLicense g = new gLicense(Utilities.append(name, " License Token"), duration, sum, downPayment);
            g.mint(1, "asdfas", 1);
            g.transfer(msg.sender, 1);
    }
...
contract gLicense { // Γ - mv - NFT - mkt - γ
    ...
    constructor(string memory _name, uint256 _duration, uint256 _sum, uint256 _downPayment) public {
        supportsInterface[0x80ac58cd] = true; // ERC721 
        supportsInterface[0x5b5e139f] = true; // METADATA
        supportsInterface[0x780e9d63] = true; // ENUMERABLE
        name = _name;
...
contract g2License { // Γ - mv - NFT - mkt - γ
    using SafeMath for uint256;
