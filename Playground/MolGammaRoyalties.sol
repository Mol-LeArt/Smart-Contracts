...
contract MolGamma { // Γ - mv - NFT - mkt - γ
    ...
    address[] public royaltiesContract;
    ...
    function mint(uint8 forSale, uint256 ethPrice, string calldata _tokenURI) external {
        ...
        royaltiesContract.push(address(g));
    ...
    function mintGAMMMA() external payable returns (address) {
        GAMMA g = new GAMMA();
        g.mint(1, "asdfas", true);
        g.transfer(msg.sender, 1);
        // address(uint160(address(g)));

        address payable wallet = address(uint160(address(g)));
    ...
    function payRoyalties() external payable {
        // GAMMA g = GAMMA(royaltiesContract[0]);


        // address(uint160(royaltiesContract[0])).transfer(msg.value);
...
contract GAMMA { // Γ - mv - NFT - mkt - γ
    ...
    function transferRoyalties() external payable {
